...
/**
 * Utility class to perform operation (get/wait for/verify/set/delete) on znode
 * in ZooKeeper
...
public class MetaTableLocator
{
    ...
    public List<Pair<HRegionInfo, ServerName>> getMetaRegionsAndLocations(ZooKeeperWatcher zkw, int replicaId) {
        ...
        list.add(new Pair<HRegionInfo, ServerName>(
                RegionReplicaUtil.getRegionInfoForReplica(HRegionInfo.FIRST_META_REGIONINFO, replicaId), serverName));
        return list;
    }
     ...
     * set up in zk.
     * 
     * @see #waitMetaRegionLocation(org.apache.hadoop.hbase.zookeeper.ZooKeeperWatcher,
     *      long)
     * @throws InterruptedException
     *             if interrupted while waiting
     */
     ...
     * @throws InterruptedException
     */
    public boolean verifyMetaRegionLocation(HConnection hConnection, ZooKeeperWatcher zkw, final long timeout)
     ...
     */
    public boolean verifyMetaRegionLocation(HConnection hConnection, ZooKeeperWatcher zkw, final long timeout,
            int replicaId) throws InterruptedException, IOException {
        ...
        try {
            ...
            // Pass -- server name doesn't resolve so it can't be assigned
            // anything.
        }
        ...
        return (service != null)
                && verifyRegionLocation(service, getMetaRegionLocation(zkw, replicaId), RegionReplicaUtil
     ...
     * 
     * @param hostingServer
     *            Interface to the server hosting <code>regionName</code>
     * @param address
     *            The servername that goes with the <code>metaServer</code>
     *            Interface. Used logging.
     * @param regionName
     *            The regionname we are interested in.
     * @return True if we were able to verify the region located at other side
     *         of
     *         the Interface.
     * @throws IOException
     */
     ...
     * <p>
     * WARNING: Does not retry. Use an
     * {@link org.apache.hadoop.hbase.client.HTable} instead.
     * 
     * @param hConnection
     * @param zkw
     * @param timeout
     *            How long to wait on meta location
     * @param replicaId
     ...
     * @throws IOException
     */
    private AdminService.BlockingInterface getMetaServerConnection(HConnection hConnection, ZooKeeperWatcher zkw,
            long timeout, int replicaId) throws InterruptedException, NotAllMetaRegionsOnlineException, IOException {
        return getCachedConnection(hConnection, waitMetaRegionLocation(zkw, replicaId, timeout));
    }

    /**
     * @param sn
     *            ServerName to get a connection against.
    ...
    private static AdminService.BlockingInterface getCachedConnection(HConnection hConnection, ServerName sn)
        ...
        try {
            ...
            if (e.getCause() != null && e.getCause() instanceof ConnectException) {
                // Catch this; presume it means the cached connection has gone
                // bad.
            }
            else {
                throw e;
     ...
     *            zookeeper reference
     * @param serverName
     *            The server hosting <code>hbase:meta</code>
     * @param state
     *            The region transition state
     * @throws KeeperException
     *             unexpected zookeeper exception
     */
    public static void setMetaLocation(ZooKeeperWatcher zookeeper, ServerName serverName, RegionState.State state)
            throws KeeperException {
     ...
     * @throws KeeperException
     */
    public static void setMetaLocation(ZooKeeperWatcher zookeeper, ServerName serverName, int replicaId,
        ...
        // the znode content.
        MetaRegionServer pbrsr = MetaRegionServer.newBuilder().setServer(ProtobufUtil.toServerName(serverName))
    ...
    public static RegionState getMetaRegionState(ZooKeeperWatcher zkw, int replicaId) throws KeeperException {
        ...
        try {
            ...
            if (data != null && data.length > 0 && ProtobufUtil.isPBMagicPrefix(data)) {
                try {
                    ...
                    ZooKeeperProtos.MetaRegionServer rl = ZooKeeperProtos.MetaRegionServer.PARSER.parseFrom(data,
        ...
        return new RegionState(RegionReplicaUtil.getRegionInfoForReplica(HRegionInfo.FIRST_META_REGIONINFO, replicaId),
                state, serverName);
    }
     ...
     * @throws InterruptedException
     */
    public List<ServerName> blockUntilAvailable(final ZooKeeperWatcher zkw, final long timeout, Configuration conf)
            throws InterruptedException {
    ...
    public ServerName blockUntilAvailable(final ZooKeeperWatcher zkw, int replicaId, final long timeout)
        ...
        try {
            while (true) {
                ...
                if (sn != null || sw.elapsed(TimeUnit.MILLISECONDS) > timeout - HConstants.SOCKET_RETRY_WAIT_MS) {
                    break;
                }
